...
func get_shape_trajectory(shape_params: Physics2DShapeQueryParameters, D_CHECK_MOTION = false) -> Array:
	...
	if motion_result[0] < 1:
		# collided on the way
		if D_CHECK_MOTION and motion_result[0] < Math.EPSILON:
			push_error("Motion distance is 0 again! [%s, %s]" % motion_result)
			return []
...
#		shape_params.transform.origin += collision_normal * 5
		var new_dir = dir.bounce(collision_normal)
		shape_params.motion = new_dir * remaining_dist
		var point = {
			"global_position": shape_params.transform.origin, #coll_center_global_position,
			"collided": true,
